#!/usr/bin/env node
import { BlocksApi, Configuration } from "@madisonbullard/notion-api-client";
...
const headers = {
	Authorization: `Bearer ${
		// biome-ignore lint/style/noNonNullAssertion: This token is guaranteed by the TUI
...
const blocksApi = new BlocksApi(new Configuration({ headers }));
...
class NotionClientWrapper {
	...
	constructor(token: string) {
		...
		this.blocksApi = new BlocksApi(
			new Configuration({ headers: this.headers }),
		);
	}

	async retrieveBlock(block_id: string): Promise<any> {
		const res = await this.blocksApi.v1BlocksIdGet({ id: block_id });
		return res;
	}
